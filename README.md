...
```c
typedef struct {
    ElementType type;           // INT / LITERAL_NAME / PRIMITIVE / EXEC_ARRAY / EXEC_NAME
    union {
        int ival;
        char name[64];
        PrimitiveFn fn;
        ExecArray *exec_array;
    } u;
} Element;
```

整数も名前もC関数もコードの塊も、この1つの型でスタックと辞書に載ります。

### `{ }` の中の名前は、実行時まで辞書を引かない

`{ 2 mul }` をコンパイルした時点では、`mul` は `"mul"` という文字列のまま
保持されます（`ELEM_EXEC_NAME`）。辞書を引くのは実際に実行される瞬間です。
...
`{ }` を読んでいる最中、`fact` はまだ辞書にありません（`def` がまだ
実行されていないため）。コンパイル時に解決する方式では失敗します。

### 組み込み演算子は辞書に関数ポインタとして登録する

`eval` は `add` や `if` を特別扱いしません。実行可能名が来たら辞書を引き、
出てきたものが関数ポインタなら呼び、コードの塊なら実行し、値なら積む——
それだけです。演算子を追加しても `eval` は変更不要です。

`if` / `ifelse` も同じ仕組みの上にあります。**制御構造が言語の構文ではなく、
ただの関数**である、というのがスタック指向言語の特徴です。
